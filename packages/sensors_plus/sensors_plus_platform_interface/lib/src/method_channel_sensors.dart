...
class MethodChannelSensors extends SensorsPlatform {
  ...
  Stream<AccelerometerEvent> get accelerometerEvents {
    _accelerometerEvents ??= _accelerometerEventChannel
        .receiveBroadcastStream(sampleRate ?? sampleRateDefault)
        .map((dynamic event) {
  ...
  Stream<GyroscopeEvent> get gyroscopeEvents {
    _gyroscopeEvents ??=
        _gyroscopeEventChannel.receiveBroadcastStream(sampleRate ?? sampleRateDefault).map((dynamic event) {
      final list = event.cast<double>();
  ...
  Stream<MagnetometerEvent> get magnetometerEvents {
    _magnetometerEvents ??=
        _magnetometerEventChannel.receiveBroadcastStream(sampleRate ?? sampleRateDefault).map((dynamic event) {
      final list = event.cast<double>();
